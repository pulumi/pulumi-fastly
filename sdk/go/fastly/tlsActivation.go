...
//
// > **Warning:** This resource is for custom (uploaded) certificates. Do not pair it with a `TlsSubscription` — Fastly automatically activates TLS on a subscription's domains once the managed certificate is issued, so creating an activation for those domains fails with `400 domainId has already been taken` (and referencing `fastly_tls_subscription.<name>.certificate_id` in the same apply fails earlier with an empty certificate ID). For managed certificates, set `configurationId` on the subscription and use the `TlsActivation` data source to read the automatically-created activation — see the `TlsSubscriptionValidation` documentation for a complete example.
//
// ## Example Usage
//
// Basic usage:
...
type TlsActivation struct {
	...
	MutualAuthenticationId pulumi.StringOutput `pulumi:"mutualAuthenticationId"`
}
...
// An alphanumeric string identifying a mutual authentication.
func (o TlsActivationOutput) MutualAuthenticationId() pulumi.StringOutput {
	return o.ApplyT(func(v *TlsActivation) pulumi.StringOutput { return v.MutualAuthenticationId }).(pulumi.StringOutput)
}
